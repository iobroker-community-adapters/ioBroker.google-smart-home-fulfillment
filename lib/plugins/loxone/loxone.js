...
class Loxone {
    ...
    async gateExecuteHandler(id, execution) {
        this.adapter.log.debug(`gateExecuteHandler for ${id}...`);
        switch (execution[0].command) {
            case 'action.devices.commands.OpenClose': {
                this.adapter.setForeignState(`${id}.position`, execution[0].params.openPercent);
                break;
            }
        }
    ...
    async gateQueryHandler(id) {
        this.adapter.log.debug(`gateQueryHandler for ${id}...`);
        const positionState = await this.adapter.getForeignStateAsync(`${id}.position`);
        return {
            [id]: {
                online: true,
                status: 'SUCCESS',
                openPercent: positionState.val
            }
        };
    }
    ...
    async lightDeviceBuilder(id, device) {
        ...
        // time it changes.
        const moodList = await this.adapter.getForeignStateListAsync(`${id}.moodList`);
        this.adapter.log.debug(`moodList length for ${id}: ${moodList.val.length}`);
    ...
    async lightQueryHandler(id) {
        ...
        // TODO: this should probably use the masterValue on/off states but they seem broken
        const activeMoods = await this.adapter.getForeignStateListAsync(`${id}.activeMoods`);
    ...
    async getDevices() {
        ...
        for (const [id, device] of Object.entries(srcObjects)) {
            ...
            if (typeof (device.native.type) === 'undefined' && typeof (device.native.control) === 'undefined') {
                // Looks like an unsupported device
                this.adapter.log.debug(`Skipping unsupported ${id}`);
                continue;
            }
            ...
            switch (deviceType) {
                case 'Gate': {
                    ...
                    this.devices[id] = {
                        handlers: {
                            onExecute: this.gateExecuteHandler.bind(this),
                            // Supply a query handler even though this appears never to be called :()
                            onQuery: this.gateQueryHandler.bind(this)
                        },
                        gDevice: {
                            id: id,
                            type: 'action.devices.types.GATE',
                            // TODO: guess we could implement some kind of locking using the Loxone prevent open/close functions?
