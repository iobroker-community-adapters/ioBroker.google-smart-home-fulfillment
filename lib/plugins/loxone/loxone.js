...
class Loxone {
    ...
    async getDevices() {
        ...
        for (const [id, device] of Object.entries(srcObjects)) {
            ...
            if (this.instanceSettings.pluginsLoxoneExcludeFunctions.indexOf(getFunction(device)) >= 0) {
                this.adapter.log.debug(`Skipping excluded ${device.native.type} ${id}`);
                continue;
            }
            ...
            switch (device.native.type) {
                ...
                case 'Switch': {
                    ...
                    this.devices[id] = {
                        handlers: {
                            onExecute: (id, execution) => { return this.switchExecuteHandler(id, execution); },
                            onQuery: (id) => { return this.switchQueryHandler(id); }
                        },
                        gDevice: {
                            id: id,
                ...
                default: {
                    this.adapter.log.debug(`Skipping unhandled ${device.native.type} ${id}`);
                }
            }
        }
    ...
    async init() {
        ...
        // TODO: The below should be on settings screen
        this.instanceSettings.pluginsLoxoneSceneReversible = false;
        this.instanceSettings.pluginsLoxoneExcludeFunctions = ['Doors & Windows', 'Electricity'];
    }
}
